...
def test_eq():
    ...
    assert hash(sequence_a) == hash(sequence_a)
    assert hash(sequence_a) == hash(sequence_b)
    assert hash(sequence_a) == hash(sequence_e)
    assert hash(sequence_a) != hash(sequence_c)
    assert hash(sequence_a) != hash(sequence_d)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ed13_3, [-2, 8, 15, 12, 66], [0, 10, 17, 14, 68]),
        (ed13_3, [], []),
    ]
)
def test_to_interval_fan_no_param(tuning, input_pi, interval_diffs):
    ...
    assert seq.to_interval_fan() == result


@pytest.mark.parametrize(
    'tuning, input_pi, ref_pi, interval_diffs',
    [
        (edo12, [3, 7, 6, 8], 3, [0, 4, 3, 5]),
        (edo31, [4, 8, 10, 22, 23], 2, [2, 6, 8, 20, 21]),
        (ed13_3, [-2, 8, 15, 10, 66], -1, [-1, 9, 16, 11, 67]),
        (ed13_3, [], 3, []),
    ]
)
def test_to_interval_fan_reference_param(
    ...
    assert seq.to_interval_fan(ref) == result
