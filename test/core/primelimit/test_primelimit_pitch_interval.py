...
@pytest.mark.parametrize(
    ...
    [
        (
            PrimeLimitTuning(5),
            (-1, 0, 1),
            'PrimeLimitPitchInterval(5/2, 5-Limit)'
        ),
        (
            PrimeLimitTuning(5),
            (-2, 1, 0),
            'PrimeLimitPitchInterval(3/4, 5-Limit)'
        ),
        (
            PrimeLimitTuning(7),
            (-2, 1, 0, 1),
            'PrimeLimitPitchInterval(21/4, 7-Limit)'
        ),
        (
            PrimeLimitTuning(3),
            (-6, 4),
            'PrimeLimitPitchInterval(81/64, 3-Limit)'
        ),
    ]

)
def test_repr(tuning, lp_vec, repr_str):
    assert repr(tuning.diff_interval(tuning.lattice.point(lp_vec))) == repr_str
...
def test_ratio_interval(tuning, ratio):
    interval = tuning.ratio_interval(ratio)
...
def test_ratio_interval_limit_violated(tuning, ratio):

    with pytest.raises(ValueError) as exc_info:
        tuning.ratio_interval(ratio)
...
def test_rs_interval(tuning, ratio_str, ratio):
    interval = tuning.rs_interval(ratio_str)
    assert interval.frequency_ratio == ratio
...
def test_rs_interval_limit_violated(tuning, ratio_str):

    with pytest.raises(ValueError) as exc_info:
        tuning.rs_interval(ratio_str)
    assert 'surpasses prime limit' in exc_info.value.args[0]
...
def test_rs_interval_bogus(tuning, ratio_str):
