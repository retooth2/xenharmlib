...
class NoteIntervalFan(IntervalFan[IndexT, NoteIntervalT]):
    """
    Base class for note interval fans
    ...
    def tuning(self):
        """
        The tuning associated with this note interval fan
        """
        return self.notation.tuning

    @property
    def pitch_interval_fan(self):
        """
        Returns the underlying pitch interval fan
        """
        return self.tuning.interval_fan(
    ...
    def is_notated_same(self, other) -> bool:
        """
        Returns True, if this interval fan is notated the same
        way as the other fan, False otherwise

        :param other: Another interval fan to compare
        """
        ...
        if other.notation is not self.notation:
            raise IncompatibleOriginContexts(
                'Interval fans must originate from the same '
                'notation context'
            )
...
class NatAccNoteIntervalFan(
    NoteIntervalFan[PeriodicIndexT, NatAccNoteIntervalT]
):
    """
    Base class for natural/accidental notation interval fans
